...
double get_sign(const char* from, const char* child_name){
  double sign;
  if (((strcmp("parent", from) == 0) && (strcmp(child_name, "left") == 0))
      or ((strcmp("child", from) == 0) && (strcmp(child_name, "right") == 0)))
    sign = -1;
  else
    sign = 1;
...
Pair parent_line_intersection(double p_x, double p_y, double c_x, double c_y, double r, const char* child_name) {
...
Pair child_line_intersection(double p_x, double p_y, double c_x, double c_y, double r, const char* child_name) {
  double m = (c_y - p_y) / (c_x - p_x);
